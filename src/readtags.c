...
static tagResult is_member(const tagEntry *entry,const char *const k,const char* const v)
{
    int i=0;
	for (i = 0  ;  i < entry->fields.count  ;  ++i)
	{
	    if (struppercmp(entry->fields.list[i].key,k)==0) {
		if (struppercmp(entry->fields.list[i].value,v)==0)
		{
		    return TagSuccess;
		}
	    }
		...
		printf("%s\n",entry->fields.list[i].key);
	    }
#endif
	}
	return TagFailure;
}
static int is_define ( const tagEntry* entry,const char* const kind)
{
    int result=0;
    const char *ptr = kind;
    	if ( entry->kind==NULL || entry->kind[0]=='\0') return 0;
	do
	{
		result = result | (int)entry->kind[0]==(int)*ptr;
	} while ( *ptr++ != '\0' && result!=1);
	return result;
}
static void findTag (const char *const name, const int options)
{
	...
	if (tagsFind (file, &entry, name, options) == TagSuccess)
	{
		...
		if (Kind!=NULL)
		{
		    result = result & is_define(&entry,Kind);
		}
		if (Key != NULL)
		{
		    result = result & is_member(&entry,Key,Value);
		}
		if (result)
		{
		    printTag (&entry);
	    ...
	    } while (tagsFindNext (file, &entry) == TagSuccess&&(max_lines!=0));
	}
	tagsClose (file);
    }
}
static void findMembers(const char *const k,const char* const v,
	const char *const name, const int options)
{
    tagFileInfo info;
    tagEntry entry;
	...
	if (tagsFind (file, &entry, name, options) == TagSuccess)
	{
	    do
	    {
		if (is_member(&entry,k,v)) 
		{
		    if ( Kind!=NULL ) {
			if (is_define(&entry,Kind)) {
			printTag (&entry);
			max_lines --;
			}
		    }
...
static void findTag4Kind(const char *const k,
	const char *const name, const int options)
{
    tagFileInfo info;
    tagEntry entry;
...
static void listTags (void)
{
    ...
    {
	while (tagsNext (file, &entry) == TagSuccess && max_lines!=0)
	{
	    result = 1;
	    if (Kind!=NULL)
	    {
...
static void listMembers (const char* const k , const char* const v)
{
    tagFileInfo info;
    tagEntry entry;
    tagFile *const file = tagsOpen (TagFileName, &info);
...
int main( int argc, char* argv[] )
{
	...
	if (arg [0] != '-')
	{
	    /*
	    if (Key !=NULL) {
		findMembers(Key,Value,arg,options);
	    }
	    else if (Kind!=NULL) {
		findTag4Kind(Kind,arg,options);
	    }
	    else {
	    }
	    */
    ...
    if (Key!= NULL && !actionSupplied) 
    {
	listMembers(Key,Value);
	actionSupplied = 1;
    }
    if (! actionSupplied)
    {
